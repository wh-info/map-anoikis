...
const THERA_COLORS = {
  small:   '#1c80ee',   // Eve-Scout's frigate-hole size string (A009, E004, …)
  frigate: '#1c80ee',   // defensive alias — in case the enum ever uses it
  medium:  '#36cccc',
  large:   '#d6d9cc',
  capital: '#f0a800',
  xlarge:  '#f0a800',
...
function buildScatterLegend() {
  ...
  const bottomLabels = ['Pods/Shuttles', 'POS/Depl.'];
...
function formatIsk(v) {
  if (v >= 1e9) return '<b class="isk-num">' + (v / 1e9).toFixed(1) + '</b>B';
  if (v >= 1e6) return '<b class="isk-num">' + (v / 1e6).toFixed(0) + '</b>M';
  if (v >= 1e3) return '<b class="isk-num">' + (v / 1e3).toFixed(0) + '</b>K';
  return '<b class="isk-num">' + v + '</b>';
}
